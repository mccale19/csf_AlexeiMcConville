...
def friends_of_friends(graph, user):
    ...
    for x in n:
        ...
        """
        for y in p:
            if y not in n and y is not user:
                s.add(y)
...
def number_of_common_friends_map(graph, user):
    ...
    for x in m:
        if x not in n and x is not user:
            y = len(common_friends(graph, user, x))
            if y > 0:
                u[x] = y
...
assert number_of_common_friends_map(rj, "Mercutio") == { 'Benvolio': 1,
    'Capulet': 2, 'Friar Laurence': 1, 'Juliet': 1, 'Montague': 2 }
...
assert number_map_to_sorted_list({"a":5, "b":2, "c":7, "d":5, "e":5}) == ['c',
    'a', 'd', 'e', 'b']
...
assert recommend_by_number_of_common_friends(rj, "Mercutio") == ['Capulet',
    'Montague', 'Benvolio', 'Friar Laurence', 'Juliet']
...
assert influence_map(rj, "Mercutio") == { 'Benvolio': 0.2,
    'Capulet': 0.5833333333333333, 'Friar Laurence': 0.2, 'Juliet': 0.2,
    'Montague': 0.45 }
